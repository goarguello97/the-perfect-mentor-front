...
import { getMessages, getUserMessages, setActiveChat } from "../features/chat/chatSlice"
        ...
        {chats?.map((chat, i) => (
          <div key={i} className="flex flex-col items-center justify-center bg-[#F5F6F7] rounded-[10px] mx-[5px]! mb-[5px]! cursor-pointer shadow-[0px_4px_4px_0px_#4444444D]"
            ...
            <div className="w-full flex mx-[5px]! my-[2px]! px-[5px]!">
              ...
              <div className={`w-10 text-center rounded-full ${chat.unreadCount > 0 && "bg-[#E61587]"}`}>{ chat.unreadCount > 0 && chat.unreadCount}</div>
          </div>
        </div>
      ))}
      </motion.div>
...
const ChatBar = () => {  
  ...
  const { chats, status, unreadMessages } = useAppSelector(state => state.chat)
  ...
  return (
    <>
      <div className="md:hidden fixed top-[15px] right-[15px] w-auto px-[10px]! py-[4px]! bg-[#CCCCCC] rounded-[40px] shadow-[0px_0px_0px_0px_#0013331A,0px_2px_5px_0px_#0013331A,0px_9px_9px_0px_#00133317,0px_21px_13px_0px_#0013330D,0px_38px_15px_0px_#00133303,0px_59px_17px_0px_#00133300] z-40" onClick={() => setView(!view)}>
        <CiChat1 size={40} />
        <div className={`absolute -translate-x-1/2 -translate-y-1/2 left-1/2 top-[46%] ${unreadMessages > 0 && "bg-[#E61587]"} w-[15px] h-[15px] rounded-full flex items-center justify-center text-[12px]`}>
        {view? "X" : unreadMessages }
        </div>
        {view &&
        <ChatList setView={setView} chats={chats} status={status} user={user} count={unreadMessages} />
        }
      </div>
    
      <div className="hidden md:flex fixed bottom-[40px] right-[40px] w-auto px-[10px]! py-[4px]! bg-[#CCCCCC] rounded-[40px] shadow-[0px_0px_0px_0px_#0013331A,0px_2px_5px_0px_#0013331A,0px_9px_9px_0px_#00133317,0px_21px_13px_0px_#0013330D,0px_38px_15px_0px_#00133303,0px_59px_17px_0px_#00133300] cursor-pointer z-40" onClick={() => setView(!view)}>
      <CiChat1 size={40} />
      <div className={`absolute -translate-x-1/2 -translate-y-1/2 left-1/2 top-[46%] ${unreadMessages > 0 && "bg-[#E61587]"} w-[15px] h-[15px] rounded-full flex items-center justify-center text-[12px]`}>
        {view? "X" : unreadMessages }
      </div>
