...
const ReporModal = ({ isOpen, onClose }) => {
  ...
  const { formErrors, handleSubmit, values, handleChange } = useForm(REPORT_INITIAL_VALUES, addReport, reportValidation)
    ...
    <>
      <div className="flex w-[100dvw] h-[100dvh] flex-col items-center justify-center md:hidden fixed inset-0 z-20 left-1/2 -translate-x-1/2 top-1/2 -translate-y-1/2 backdrop-blur-sm z-50">
        <div className="bg-[#FFFFFF] w-[calc(100%-20px)] h-[90%] rounded-[40px] shadow-xl overflow-hidden px-[20px]! flex flex-col">
          ...
          <h3 className="text-[20px] font-semibold text-[#444444]">Enviar reporte nuevo</h3>
          <button onClick={onClose} className="text-[#444444] hover:text-gray-600 transition-colors cursor-pointer">
          ...
          <label className="text-[15px] text-[#3A3D46] w-[calc(100%-10px)] mb-[10px]!">Asunto: <br />
            <input type="text" className="w-full h-[30px] bg-[#FFFFFF] border-2 border-[#444444] rounded-[20px] px-[20px]!" />
          </label>
          <label className="text-[15px] text-[#3A3D46] w-[calc(100%-10px)] mb-[10px]!">Descripción: </label>
          <textarea name="" id="" className="w-[calc(100%-10px)] h-full bg-[#FFFFFF] border-2 border-[#444444] rounded-[20px] mb-[20px]! py-[10px]! px-[20px]!"></textarea>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col w-full flex-1 items-center justify-center">
          <label className="text-[15px] text-[#3A3D46] w-[calc(100%-10px)] mb-[10px]!">Destinatario: <br />
              {status.matches === "loading" ? 
                <>Cargando...</>
              :
                <select name="receiverId" value={values.receiverId} onChange={handleChange} className="font-bold text-[15px] text-[#444444] bg-[#FFFFFF] border-2 border-[#444444] rounded-[20px] w-full  h-[30px] mt-[5px]! px-[20px]" required>
                  <option className="font-bold text-[15px] text-[#444444] w-full my-[5px]!">Seleccione una opción</option>
                {matches?.map((match, i) => (
                  <option key={i} value={match._id} className="font-bold text-[15px] text-[#444444] w-full my-[5px]!">{match.fullname.charAt(0).toUpperCase()+match.fullname.slice(1).toLowerCase()}</option>
                ))}
            </select>}
          </label>
          <label className="text-[15px] text-[#3A3D46] w-[calc(100%-10px)] mb-[10px]!">Asunto: <br />
            <input type="text" name="issue" value={values.issue} onChange={handleChange}  className="w-full h-[30px] bg-[#FFFFFF] border-2 border-[#444444] rounded-[20px] px-[20px]!" />
          </label>
          <label className="text-[15px] text-[#3A3D46] w-[calc(100%-10px)] mb-[10px]!">Descripción: </label>
          <textarea name="content" value={values.content} onChange={handleChange} className="w-[calc(100%-10px)] h-full bg-[#FFFFFF] border-2 border-[#444444] rounded-[20px] mb-[20px]! py-[10px]! px-[20px]! required"></textarea>
