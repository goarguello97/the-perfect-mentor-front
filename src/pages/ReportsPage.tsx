  ...
  return (
    <>
      <div className="flex w-full h-full flex-col items-center md:hidden">
        ...
        <ReporModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} />
        <div className="w-[calc(100%-20px)] mx-[10px] mt-[35px]! pt-[20px]! flex-1 bg-[#FFFFFF] rounded-t-[40px] shadow-[0px_4px_4px_0px_#4444444D] flex flex-col items-center justify-start gap-[5px]">
          <button onClick={() => setIsModalOpen(true)} className="h-[55px]! rounded-[40px] text-[15px] leading-[55px] text-center font-bold border border-[#44444426] text-[#44444480] px-[10px]! mb-[15px]!"
          >Nuevo Reporte
          </button>
          ...
          {reports?.map((report, i) => (
            <div className={`w-[calc(100%-40px)] h-[119px] ${report.answered ? "bg-[#39B54A1A]" : "bg-[#E615871A]"} rounded-[20px] relative flex flex-col items-start justify-center p-[15px]!`}>
              ...
              <div className={`w-[6px] h-[6px] rounded-full ms-[5px]! ${report.answered? "bg-[#39B54A]" : "bg-[#E61587]"}`}></div>
            </div>
            ...
            <p className=" h-[68px] text-[14px] text-[#444444] leading-[17px]">
              <span className="font-bold">Rol:</span> {report.receiverId.role.role.charAt().toUpperCase() + report.receiverId.role.role.slice(1).toLowerCase()} <br />
              <span className="font-bold">Motivo:</span> {report.issue} <br />
              <span className="font-bold">Email:</span> {report.receiverId.email}
              <br />
                ...
                <div className="flex flex-col flex-1 shadow-[0px_4px_4px_0px_#00000040] rounded-[20px] overflow-hidden">
                  ...
                  <div className="flex-1 min-h-0 overflow-y-auto flex flex-col gap-[6px]">
                    ...
                    {reports?.map((report, i) => (
                      ...
                      <div className="text-[14px] text-[#444444] text-center">
                        {report.receiverId.role.role.charAt(0).toUpperCase()+report.receiverId.role.role.slice(1).toLowerCase()}
                      </div>
                      <div className="text-[14px] text-[#444444] text-center">
                      ...
                      <div>
                        <div className={`w-[100px] h-[20px] flex items-center justify-center ${report.answered? "bg-[#39B54A33]" : "bg-[#E6158733]"} rounded-[40px] px-[7px]! mx-auto!`}>
                          <span className={`h-[20x] leading-[20px] text-[12px] ${report.answered ? "text-[#39B54A]" : "text-[#E61587]"}`}>
                            {report.answered? "Verificado" : "No verificado" }
                          </span>
