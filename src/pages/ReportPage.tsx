...
const Toggle = ({answered, reportId}:{answered:boolean, reportId:string}) => {
  const dispatch = useAppDispatch()
    ...
    return (
      <label className="w-full flex items-center cursor-pointer justify-end gap-3">
        <div className="relative">
          ...
          />
          <div className={`block w-12 h-7 rounded-full transition-colors ${answered ? 'bg-[#39B54A1A]' : 'bg-[#E615871A]'}`}></div>
         
          <div className={`absolute left-1 top-1 bg-white w-5 h-5 rounded-full transition-transform ${answered ? 'translate-x-5' : 'translate-x-0'}`}></div>
        </div>
